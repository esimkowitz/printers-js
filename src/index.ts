...
export interface Printer extends Readonly<NativePrinter> {
  ...
  // Job tracking methods (all async due to lazy module loading)
...
export interface PrinterClass {
  fromName(name: string): Promise<Printer | null>;
  new (): never;
}
...
// N-API module lazy loading - no top-level await
// This allows the module to be imported in CJS contexts and bundlers that don't support TLA
 ...
 * @returns Platform identifier string (e.g., "darwin-arm64", "win32-x64-msvc")
 */
...
/**
 * Load the native N-API module for the current platform.
 * This is called lazily on first use.
 */
async function loadNativeModule(): Promise<NativeModule> {
  // Log simulation mode on first load
  if (isSimulationMode && !simulationModeLogged) {
  ...
  try {
    ...
    return await import(`../npm/${platformString}/index.js`);
  } catch (localError) {
...
/**
 * Get the native module, loading it lazily if needed.
 * Uses memoization to ensure the module is only loaded once.
 * @returns Promise resolving to the native module
 */
function getNativeModule(): Promise<NativeModule> {
  // Return cached module immediately if available
  if (nativeModuleCache) {
    return Promise.resolve(nativeModuleCache);
  }

  // Start loading if not already in progress
  if (!nativeModulePromise) {
    nativeModulePromise = loadNativeModule()
      .then(mod => {
...
class PrinterWrapperImpl {
  ...
  async exists(): Promise<boolean> {
    return await printerExists(this._native.name);
  }
   ...
   * @returns Promise resolving to array of active PrinterJob objects
   */
   ...
   * @returns Promise resolving to array of completed/cancelled PrinterJob objects
   */
   ...
   * @returns Promise resolving to PrinterJob object if found, null otherwise
   */
   ...
   * @returns Promise resolving to array of all PrinterJob objects
   */
 ...
 * @returns Promise resolving to array of Printer objects
 */
 ...
 */
export async function getAllPrinterNames(): Promise<string[]> {
  try {
    const nativeModule = await getNativeModule();
 ...
 * @returns Promise resolving to Printer object if found, null otherwise
 */
 ...
 * @returns Promise resolving to true if printer exists, false otherwise
 */
...
export async function shutdown(): Promise<void> {
  try {
    // Only shutdown if the module was loaded
    if (nativeModuleCache) {
      nativeModuleCache.shutdown();
    }
  } catch (error) {
 ...
 * @returns Promise resolving to default printer if found, null otherwise
 */
...
export async function stopPrinterStateMonitoring(): Promise<void> {
  try {
    // Stop native monitoring if module was loaded
    if (nativeModuleCache?.stopStateMonitoring) {
      nativeModuleCache.stopStateMonitoring();
    }
...
export function isPrinterStateMonitoringActive(): boolean {
  return (
    monitoringInterval !== null ||
    Boolean(nativeModuleCache?.isStateMonitoringActive?.())
  );
}
 ...
 * @returns Promise resolving to map of printer names to their current state information
 */
export async function getPrinterStateSnapshots(): Promise<
  Map<string, PrinterStateSnapshot>
> {
...
function pollPrinterStates(): void {
  // Wrap async operation - errors are caught internally
