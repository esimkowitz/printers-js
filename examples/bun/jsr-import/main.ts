...
async function main() {
  ...
  try {
    ...
    if (existingPrinter) {
      ...
      try {
        ...
        if (error.message.includes("File not found")) {
          console.log(`   Error type: PrintError.FileNotFound (${PrintError.FileNotFound})`);
        } else if (error.message.includes("Simulated failure")) {
          console.log(`   Error type: PrintError.SimulatedFailure (${PrintError.SimulatedFailure})`);
        }
      }
    }
    ...
    console.log(`   5 printer enumeration calls took ${(endTime - startTime).toFixed(2)}ms`);
